...
export function validateEventDefinition(def: EventSchemaDraft["eventDefinitions"][number]): string | null {
...
export function validateEventSchemaSettings(d: EventSchemaDraft): string | null {
...
export function validateEventSchemaDraft(d: EventSchemaDraft): string | null {
  for (const def of d.eventDefinitions) {
    const err = validateEventDefinition(def);
    if (err) return err;
  }
  return validateEventSchemaSettings(d);
}
